...
alter PROCEDURE SP_CargaDDLFecha
	@IDESPECIALIDAD INT,
    @IDMEDICO INT,
    @IDPACIENTE INT,
    @DIA SMALLDATETIME
...
EXEC SP_CargaDDLFecha 1,1,1,'09/12/2022'
...
SELECT DISTINCT MED.ID IDMEDICO, PER.APELLIDO, PER.NOMBRE, MXE.IDESPECIALIDAD, ESP.NOMBRE AS ESPECIALIDAD, HORA_DESDE, HORA_HASTA 
            ...
            INNER JOIN MEDICO_X_HORARIO MXH ON MXH.IDMEDICO = MED.ID
            INNER JOIN HORARIO HOR ON HOR.ID = MXH.IDHORARIO
            WHERE PER.ACTIVO = 1 AND ESP.ACTIVO = 1 AND ESP.ID = 1
            AND HOR.DIA = DATEPART(weekday, '10/12/2022')
            AND MED.ID = 1